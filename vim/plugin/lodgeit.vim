" lodgeit.vim: Vim plugin for paste.plurk.com
...
srv = ServerProxy('http://paste.plurk.com/xmlrpc/', allow_none=True)
...
def paste_id_from_url(url):
    regex = re.compile(r'^http://paste.plurk.com/show/([^/]+)/?$')
    m = regex.match(url)
    if m is not None:
        return m.group(1)
...
# new paste or reply
    ...
    url = 'http://paste.plurk.com/show/%s' % paste_id
